...
async def test_ping_endpoint(client: TestClient):
...
async def test_create_client(client: TestClient, sample_client_data: dict):
    """Test crear cliente"""
...
async def test_get_clients(client: TestClient, sample_client_data: dict):
...
async def test_get_client_by_id(client: TestClient, sample_client_data: dict):
    """Test obtener cliente por ID"""
    # Crear cliente
    create_response = await client.post("/api/v1/clients/", json=sample_client_data)
    created_client_id = create_response.json()["client_id"]
    
    response = await client.get(f"/api/v1/clients/{created_client_id}")
    assert response.status_code == 200
    data = response.json()
    assert data["client_id"] == created_client_id
    assert data["name"] == sample_client_data["name"]

async def test_get_client_not_found(client: TestClient):
    """Test cliente no encontrado"""
    response = await client.get("/api/v1/clients/99999")
    assert response.status_code == 404
    assert response.json() == {"detail": "Client not found"}

async def test_update_client(client: TestClient, sample_client_data: dict):
    ...
    response = await client.put(f"/api/v1/clients/{created_client_id}", json=update_data)
    assert response.status_code == 200
    data = response.json()
...
async def test_delete_client(client: TestClient, sample_client_data: dict):
    """Test eliminar cliente"""
    # Crear cliente
    create_response = await client.post("/api/v1/clients/", json=sample_client_data)
    created_client_id = create_response.json()["client_id"]
    
    response = await client.delete(f"/api/v1/clients/{created_client_id}")
    assert response.status_code == 200
    assert response.json() == {"message": "Client deleted successfully"}
    
    # Verificar que el cliente fue eliminado
    get_response = await client.get(f"/api/v1/clients/{created_client_id}")
...
async def test_create_product(client: TestClient, sample_product_data: dict):
    """Test crear producto"""
...
async def test_get_products(client: TestClient, sample_product_data: dict):
...
async def test_get_product_by_sku(client: TestClient, sample_product_data: dict):
    """Test obtener producto por SKU"""
    await client.post("/api/v1/products/", json=sample_product_data)
    
    response = await client.get(f"/api/v1/products/{sample_product_data['sku']}")
    assert response.status_code == 200
    data = response.json()
    assert data["sku"] == sample_product_data["sku"]
...
async def test_create_warehouse(client: TestClient, sample_warehouse_data: dict):
    """Test crear almacén"""
...
async def test_get_warehouses(client: TestClient, sample_warehouse_data: dict):
    """Test obtener almacenes"""
    await client.post("/api/v1/warehouses/", json=sample_warehouse_data)
...
async def test_create_stock(client: TestClient, sample_stock_data: dict, sample_warehouse_data: dict, sample_product_data: dict):
    """Test crear registro de stock"""
    # Crear dependencias primero
    warehouse_response = await client.post("/api/v1/warehouses/", json=sample_warehouse_data)
    ...
    response = await client.post("/api/v1/stock/", json=sample_stock_data)
    assert response.status_code == 200
    data = response.json()
    assert data["quantity"] == sample_stock_data["quantity"]
    assert "stock_id" in data

async def test_get_stock_with_filters(client: TestClient, sample_stock_data: dict, sample_warehouse_data: dict, sample_product_data: dict):
    ...
    await client.post("/api/v1/stock/", json=sample_stock_data)
    
    # Test con filtros
    response = await client.get(f"/api/v1/stock/?warehouse_id={sample_stock_data['warehouse_id']}&sku={sample_stock_data['sku']}")
    assert response.status_code == 200
    data = response.json()
    assert isinstance(data, list)
    assert len(data) > 0
...
async def test_create_technician(client: TestClient, sample_technician_data: dict):
    """Test crear técnico"""
    response = await client.post("/api/v1/technicians/", json=sample_technician_data)
    assert response.status_code == 200
    data = response.json()
    assert data["name"] == sample_technician_data["name"]
    assert "technician_id" in data

async def test_get_technicians(client: TestClient, sample_technician_data: dict):
    """Test obtener técnicos"""
    await client.post("/api/v1/technicians/", json=sample_technician_data)
...
async def test_create_equipment(client: TestClient, sample_equipment_data: dict, sample_client_data: dict, sample_product_data: dict):
    """Test crear equipo instalado"""
    # Crear dependencias
    client_response = await client.post("/api/v1/clients/", json=sample_client_data)
    product_response = await client.post("/api/v1/products/", json=sample_product_data)
    
    sample_equipment_data["client_id"] = client_response.json()["client_id"]
    sample_equipment_data["sku"] = product_response.json()["sku"]
    
    response = await client.post("/api/v1/equipment/", json=sample_equipment_data)
    assert response.status_code == 200
    data = response.json()
...
async def test_create_intervention(client: TestClient, sample_intervention_data: dict, sample_client_data: dict, sample_technician_data: dict):
    """Test crear intervención"""
    # Crear dependencias
    client_response = await client.post("/api/v1/clients/", json=sample_client_data)
    technician_response = await client.post("/api/v1/technicians/", json=sample_technician_data)
    ...
    dummy_client_res = await client.post("/api/v1/clients/", json=dummy_client.model_dump())
    dummy_product_res = await client.post("/api/v1/products/", json=dummy_product.model_dump())
    ...
    dummy_equipment_res = await client.post("/api/v1/equipment/", json=dummy_equipment_data)
    sample_intervention_data["equipment_id"] = dummy_equipment_res.json()["equipment_id"]
    
    response = await client.post("/api/v1/interventions/", json=sample_intervention_data)
    assert response.status_code == 200
    data = response.json()
    assert data["type"] == sample_intervention_data["type"]
    assert "intervention_id" in data

async def test_get_interventions_by_technician(client: TestClient, sample_intervention_data: dict, sample_client_data: dict, sample_technician_data: dict):
    ...
    await client.post("/api/v1/interventions/", json=sample_intervention_data)
    
    response = await client.get(f"/api/v1/interventions/technician/{sample_intervention_data['technician_id']}")
    assert response.status_code == 200
    data = response.json()
    assert isinstance(data, list)
    assert len(data) > 0
...
async def test_create_contract(client: TestClient, sample_contract_data: dict, sample_client_data: dict):
    """Test crear contrato"""
    # Crear cliente
    client_response = await client.post("/api/v1/clients/", json=sample_client_data)
...
async def test_get_contracts_by_client(client: TestClient, sample_contract_data: dict, sample_client_data: dict):
    ...
    response = await client.get(f"/api/v1/contracts/client/{sample_contract_data['client_id']}")
    assert response.status_code == 200
    data = response.json()
    assert isinstance(data, list)
    assert len(data) > 0
...
async def test_create_chat_session(client: TestClient, sample_chat_session_data: dict, sample_client_data: dict):
    """Test crear sesión de chat"""
    # Crear cliente
    client_response = await client.post("/api/v1/clients/", json=sample_client_data)
    sample_chat_session_data["client_id"] = client_response.json()["client_id"]
    
    response = await client.post("/api/v1/chat/sessions/", json=sample_chat_session_data)
    assert response.status_code == 200
    data = response.json()
...
async def test_create_chat_message(client: TestClient, sample_chat_session_data: dict, sample_chat_message_data: dict, async_db_session: AsyncSession):
    ...
    response = await client.post("/api/v1/chat/messages/", json=sample_chat_message_data)
    assert response.status_code == 200
    data = response.json()
...
async def test_get_chat_messages_by_session(client: TestClient, sample_chat_session_data: dict, sample_chat_message_data: dict, async_db_session: AsyncSession):
    ...
    response = await client.get(f"/api/v1/chat/messages/session/{chat_id}")
    assert response.status_code == 200
    data = response.json()
    assert isinstance(data, list)
    assert len(data) > 0
...
async def test_create_knowledge_feedback(client: TestClient, sample_knowledge_feedback_data: dict):
    """Test crear feedback de conocimiento"""
    response = await client.post("/api/v1/knowledge/", json=sample_knowledge_feedback_data)
    assert response.status_code == 200
    data = response.json()
    assert data["question"] == sample_knowledge_feedback_data["question"]
    assert "feedback_id" in data

async def test_get_knowledge_feedback_by_user_type(client: TestClient, sample_knowledge_feedback_data: dict):
    """Test filtrar feedback por tipo de usuario"""
    # Crear feedback
    await client.post("/api/v1/knowledge/", json=sample_knowledge_feedback_data)
    
    response = await client.get(f"/api/v1/knowledge/?user_type={sample_knowledge_feedback_data['user_type']}")
    assert response.status_code == 200
    data = response.json()
    assert isinstance(data, list)
    assert len(data) > 0
...
async def test_get_knowledge_feedback_by_status(client: TestClient, sample_knowledge_feedback_data: dict):
    """Test filtrar feedback por estado"""
    # Crear feedback
    await client.post("/api/v1/knowledge/", json=sample_knowledge_feedback_data)
    
    response = await client.get(f"/api/v1/knowledge/?status={sample_knowledge_feedback_data['status']}")
    assert response.status_code == 200
    data = response.json()
    assert isinstance(data, list)
    assert len(data) > 0
...
async def test_clients_pagination(client: TestClient, multiple_clients_data: list):
    """Test paginación en clientes"""
    # Crear múltiples clientes
    for client_data in multiple_clients_data:
        await client.post("/api/v1/clients/", json=client_data)

    # Test paginación
    response_page_1 = await client.get("/api/v1/clients/?skip=0&limit=3")
    assert response_page_1.status_code == 200
    page_1_data = response_page_1.json()
    assert len(page_1_data) == 3

    response_page_2 = await client.get("/api/v1/clients/?skip=3&limit=3")
...
async def test_create_client_missing_required_fields(client: TestClient):
    """Test crear cliente sin campos requeridos"""
    incomplete_data = {
        "name": "Cliente Test"
        # Faltan campos requeridos
    }
    response = await client.post("/api/v1/clients/", json=incomplete_data)
    assert response.status_code == 422 # Unprocessable Entity
    # Asegúrate de que los mensajes de error de Pydantic sean los esperados
    details = response.json()["detail"]
    assert any("field required" in d["msg"] for d in details) 