...
from backend.db.models.contract_model import Contract
from backend.db.schemas.contract_schema import ContractCreate, ContractUpdate
...
class CRUDContract(CRUDBase[Contract, ContractCreate, ContractUpdate]):
    ...
    def get_by_type(self, db: Session, *, contract_type: str) -> List[Contract]:
        """Obtener contratos por tipo"""
        return db.query(Contract).filter(Contract.contract_type == contract_type).all()
