...
class CRUDStock(CRUDBase[Stock, StockCreate, StockUpdate]):
    ...
    def get_by_sku(self, db: Session, *, sku: str, skip: int = 0, limit: int = 100) -> List[Stock]:
        """Obtener todo el stock de un producto en todos los almacenes"""
        return db.query(Stock).filter(Stock.sku == sku).offset(skip).limit(limit).all()

    def get_by_warehouse(self, db: Session, *, warehouse_id: int, skip: int = 0, limit: int = 100) -> List[Stock]:
        """Obtener todo el stock de un almacén"""
        return db.query(Stock).filter(Stock.warehouse_id == warehouse_id).offset(skip).limit(limit).all()
    ...
    def get_by_product_and_warehouse(self, db: Session, *, product_id: str, warehouse_id: int) -> Optional[Stock]:
    ...
    def get_by_product(self, db: Session, *, product_id: str, skip: int = 0, limit: int = 100) -> List[Stock]:
        """Obtener todo el stock de un producto en todos los almacenes"""
        return db.query(Stock).filter(Stock.sku == product_id).offset(skip).limit(limit).all()
