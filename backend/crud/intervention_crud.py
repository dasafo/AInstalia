...
class CRUDIntervention(CRUDBase[Intervention, InterventionCreate, InterventionUpdate]):
    ...
    def get_by_client(self, db: Session, *, client_id: int, skip: int = 0, limit: int = 100) -> List[Intervention]:
        """Obtener intervenciones de un cliente"""
        return db.query(Intervention).filter(Intervention.client_id == client_id).offset(skip).limit(limit).all()

    def get_by_technician(self, db: Session, *, technician_id: int, skip: int = 0, limit: int = 100) -> List[Intervention]:
        """Obtener intervenciones de un técnico"""
        return db.query(Intervention).filter(Intervention.technician_id == technician_id).offset(skip).limit(limit).all()

    def get_by_equipment(self, db: Session, *, equipment_id: int, skip: int = 0, limit: int = 100) -> List[Intervention]:
        """Obtener intervenciones de un equipo"""
        return db.query(Intervention).filter(Intervention.equipment_id == equipment_id).offset(skip).limit(limit).all()

    def get_by_status(self, db: Session, *, status: str, skip: int = 0, limit: int = 100) -> List[Intervention]:
        """Obtener intervenciones por estado"""
        return db.query(Intervention).filter(Intervention.status == status).offset(skip).limit(limit).all()
