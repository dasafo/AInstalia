...
async def create_warehouse(
    ...
    return await warehouse_crud.create(db=db, obj_in=warehouse)
...
async def read_warehouses(
    ...
    return await warehouse_crud.get_multi(db=db, skip=skip, limit=limit)
...
async def read_warehouse(
    ...
    warehouse = await warehouse_crud.get(db=db, warehouse_id=warehouse_id)
    if not warehouse:
...
async def update_warehouse(
    ...
    if not warehouse:
        raise HTTPException(status_code=404, detail="Almacén no encontrado")
    return await warehouse_crud.update(db=db, db_obj=warehouse, obj_in=warehouse_update)
...
async def delete_warehouse(
    ...
    if not warehouse:
        raise HTTPException(status_code=404, detail="Almacén no encontrado")
    return await warehouse_crud.remove(db=db, warehouse_id=warehouse_id)