...
async def read_clients(
    ...
    return client_crud.get_multi(db=db, skip=skip, limit=limit)
...
async def read_client(
    ...
    client = client_crud.get(db=db, client_id=client_id)
    if not client:
...
async def update_client(
    ...
    return client_crud.update(db=db, db_obj=client, obj_in=client_update)
...
async def delete_client(
    ...
    if not client:
        raise HTTPException(status_code=404, detail="Cliente no encontrado")
    return client_crud.remove(db=db, client_id=client_id)