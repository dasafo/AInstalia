...
async def create_intervention(
    ...
    return intervention_crud.create(db=db, obj_in=intervention)
...
async def read_interventions(
    ...
    if technician_id:
        return intervention_crud.get_by_technician(db=db, technician_id=technician_id, skip=skip, limit=limit)
    elif status:
        return intervention_crud.get_by_status(db=db, status=status, skip=skip, limit=limit)
    else:
        return intervention_crud.get_multi(db=db, skip=skip, limit=limit)
...
async def read_intervention(
    ...
    intervention = intervention_crud.get(db=db, intervention_id=intervention_id)
    if not intervention:
...
async def update_intervention(
    ...
    if not intervention:
        raise HTTPException(status_code=404, detail="Intervención no encontrada")
    return intervention_crud.update(db=db, db_obj=intervention, obj_in=intervention_update)
...
async def delete_intervention(
    ...
    if not intervention:
        raise HTTPException(status_code=404, detail="Intervención no encontrada")
    return intervention_crud.remove(db=db, intervention_id=intervention_id)